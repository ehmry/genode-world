		...
		Genode::Reporter::Xml_generator xml(playlist_reporter, [&] () {
			...
			xml.node("trackList", [&] () {
				playlist.for_each_track([&] (Playlist::Track const &t) {
					...
					xml.node("track", [&] () {
						xml.node("location", [&] () {
							xml.append_sanitized(info.path); });
						//xml.node("identifier", [&] () {
						//	xml.append_sanitized(info.id); });
						xml.node("title", [&] () {
							xml.append_sanitized(info.title); });
						xml.node("creator", [&] () {
							xml.append_sanitized(info.artist); });
						xml.node("album", [&] () {
							xml.append_sanitized(info.album); });
						//xml.node("trackNum", [&] () {
						//	xml.append_sanitized(info.track); });
						xml.node("duration", [&] () {
